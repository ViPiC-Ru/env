...
"d.m.Y");if(a=c.fun.clear(b.serialNumber))d["SYS-SERIAL"]=a;if(a=c.fun.clear(b.description))d["SYS-DESCRIPTION"]=a;d["SYS-ARCHITECTURE"]=b.osArchitecture&&!b.osArchitecture.indexOf("64")?"x64":"x86";break}response=t.execQuery(c.fun.debug("SELECT caption, size FROM Win32_LogicalDisk WHERE driveType = 2 OR driveType = 3 OR driveType = 4"));for(e=new Enumerator(response);!e.atEnd();)b=e.item(),e.moveNext(),b.caption&&g&&g.systemDrive&&-1!=b.caption.indexOf(g.systemDrive)||d["BAK-DRIVE"]||(a=c.fun.clear(b.caption))&&
b.size>=c.val.driveMinSize&&(d["BAK-DRIVE"]=a);response=t.execQuery(c.fun.debug("SELECT manufacturer, product, serialNumber FROM Win32_BaseBoard WHERE hostingBoard = TRUE"));for(e=new Enumerator(response);!e.atEnd();){b=e.item();e.moveNext();if(a=c.fun.clear(b.product))d["PCB-NAME"]=a;a&&(a=c.fun.clear(b.manufacturer,"Inc.").replace("Hewlett-Packard","HP"))&&(d["PCB-NAME"]=a.split(" ")[0]+" "+c.fun.clear(b.product));if(a=c.fun.clear(b.serialNumber))d["PCB-SERIAL"]=a;break}response=t.execQuery(c.fun.debug("SELECT releaseDate, manufacturer, smBIOSBIOSVersion, serialNumber FROM Win32_BIOS WHERE primaryBIOS = TRUE"));
for(e=new Enumerator(response);!e.atEnd();){b=e.item();e.moveNext();if(a=c.fun.clear(b.releaseDate))d["PCB-BIOS-RELEASE"]=c.lib.date2str(c.fun.wql2date(a,r.getTimezoneOffset()),"d.m.Y H:i:s");if(a=c.fun.clear(b.releaseDate))d["PCB-BIOS-RELEASE-DATE"]=c.lib.date2str(c.fun.wql2date(a,r.getTimezoneOffset()),"d.m.Y");if(a=c.fun.clear(b.manufacturer,"Inc."))d["PCB-BIOS-MANUFACTURE"]=a;if(a=c.fun.clear(b.smBIOSBIOSVersion))d["PCB-BIOS-VERSION"]=a;if(a=c.fun.clear(b.serialNumber))d["PCB-BIOS-SERIAL"]=a;
break}g=null;response=t.execQuery(c.fun.debug("SELECT * FROM Win32_NetworkAdapterConfiguration WHERE ipEnabled = TRUE AND fullDNSRegistrationEnabled = TRUE"));for(e=new Enumerator(response);!e.atEnd();)if(b=e.item(),e.moveNext(),!b.serviceName||-1==b.serviceName.indexOf("vpn"))if(!b.serviceName||-1==b.serviceName.indexOf("loop"))if(!b.serviceName||-1==b.serviceName.indexOf("VBox")){b.interfaceIndex&&(g=b);if(null!=b.ipAddress){f=b.ipAddress.toArray();h=0;for(var q=f.length;h<q;h++)(a=c.fun.clear(f[h]))&&
...
(d["NET-DNS-V4"]=a),a&&-1==a.indexOf(".")&&!d["NET-DNS-V6"]&&(d["NET-DNS-V6"]=a);if(null!=b.ipSubnet)for(f=b.ipSubnet.toArray(),h=0,q=f.length;h<q;h++)(a=c.fun.clear(f[h]))&&-1!=a.indexOf(".")&&!d["NET-SUBNET-V4"]&&(d["NET-SUBNET-V4"]=a),a&&-1==a.indexOf(".")&&!d["NET-SUBNET-V6"]&&(d["NET-SUBNET-V6"]=a);if(a=c.fun.clear(b.dhcpServer))d["NET-DHCP-V4"]=a;if(a=c.fun.clear(b.description,"Сетевой адаптер","Адаптер","для виртуальной сети","Сетевая карта","Контроллер","NIC (NDIS 6.20)","- Минипорт планировщика пакетов",
"Family Controller","Adapter","Virtual Miniport","for Windows","x64").replace(/#\d+$/g,""))d["NET-NAME"]=a;if(a=c.fun.clear(b.macAddress))d["NET-MAC"]=a;break}if(g&&g.interfaceIndex)for(response=t.execQuery(c.fun.debug("SELECT speed, timeOfLastReset FROM Win32_NetworkAdapter WHERE netEnabled = TRUE AND interfaceIndex = "+c.fun.repair(g.interfaceIndex))),e=new Enumerator(response);!e.atEnd();){b=e.item();e.moveNext();if(a=b.speed)d["NET-SPEED"]=c.fun.info2str(a,0,1E3)+"bps";if(a=b.speed)d["NET-SPEED-VAL"]=
a;if(a=b.timeOfLastReset)d["NET-RESET"]=c.lib.date2str(c.fun.wql2date(a),"d.m.Y H:i:s");if(a=b.timeOfLastReset)d["NET-RESET-DATE"]=c.lib.date2str(c.fun.wql2date(a),"d.m.Y");break}response=t.execQuery(c.fun.debug("SELECT * FROM Win32_ComputerSystem"));for(e=new Enumerator(response);!e.atEnd();){b=e.item();e.moveNext();if(a=b.dnsHostName)m=a;if(a=b.name)m||(m=a.toLowerCase());b.domain!=b.workgroup&&(p=b.domain);if(a=b.userName)l.domain=a.split(c.val.keyDelim)[0];if(a=b.userName)l.login=a.split(c.val.keyDelim)[1];
if(a=b.userName)l.account=a;if(a=c.fun.clear(m))d["NET-HOST"]=a;if(a=c.fun.clear(b.domain))d["NET-DOMAIN"]=a;if(a=c.fun.clear(b.model))d["DEV-NAME"]=a;a&&(a=c.fun.clear(b.manufacturer,"Inc.","Hewlett-Packard"))&&(d["DEV-NAME"]=a.split(" ")[0]+" "+c.fun.clear(b.model));break}!l.account&&v&&(f=[],e=v.methods_.item("getStringValue"),b=e.inParameters.spawnInstance_(),b.hDefKey=2147483650,b.sSubKeyName="SOFTWARE\\Microsoft\\Windows NT\\CurrentVersion\\Winlogon",b.sValueName=c.fun.debug("DefaultDomainName"),
b=v.execMethod_(e.name,b),!b.returnValue&&b.sValue&&f.push(b.sValue),e=v.methods_.item("getStringValue"),b=e.inParameters.spawnInstance_(),b.hDefKey=2147483650,b.sSubKeyName="SOFTWARE\\Microsoft\\Windows NT\\CurrentVersion\\Winlogon",b.sValueName=c.fun.debug("DefaultUserName"),b=v.execMethod_(e.name,b),!b.returnValue&&b.sValue&&f.push(b.sValue),2==f.length&&(l.domain=f[0]),2==f.length&&(l.login=f[1]),2==f.length&&(l.account=f.join(c.val.keyDelim)));if(!l.account){f=null;response=t.execQuery(c.fun.debug("SELECT lastUseTime, localPath, loaded, sid FROM Win32_UserProfile WHERE special = FALSE"));
for(e=new Enumerator(response);!e.atEnd();)b=e.item(),e.moveNext(),(a=f?0:1)||(a=b.loaded&&!f.loaded?1:a),a||(a=c.lib.compare(c.fun.wql2date(b.lastUseTime),c.fun.wql2date(f.lastUseTime))),0<a&&(f=b);if(b=f)if(l.profile=b.localPath,a=b.sid)l.sid=a}if(!l.account&&l.sid){f=[];try{b=t.get('Win32_SID.SID="'+l.sid+'"')}catch(y){b=null}(a=b.referencedDomainName)&&f.push(a);(a=b.accountName)&&f.push(a);2==f.length&&(l.domain=f[0]);2==f.length&&(l.login=f[1]);2==f.length&&(l.account=f.join(c.val.keyDelim))}if(l.account&&
c.lib.hasValue([".",m],l.domain,!1))for(response=t.execQuery(c.fun.debug("SELECT domain, name, fullName, sid FROM Win32_UserAccount WHERE name = "+c.fun.repair(l.login)+" AND domain = "+c.fun.repair(l.domain))),e=new Enumerator(response);!e.atEnd();){b=e.item();e.moveNext();f=[];l.name=b.fullName;if(a=b.sid)l.sid=a;(a=b.domain)&&f.push(a);(a=b.name)&&f.push(a);2==f.length&&(l.domain=f[0]);2==f.length&&(l.login=f[1]);2==f.length&&(l.account=f.join(c.val.keyDelim));break}if(l.account&&u&&!c.lib.hasValue([".",
m],l.domain,!1))for(f="DS_co DS_c DS_company DS_displayName DS_department DS_info DS_homeDirectory DS_l DS_mail DS_mobile DS_objectSid DS_telephoneNumber DS_title DS_distinguishedName".split(" "),response=u.execQuery(c.fun.debug("SELECT "+f.join(", ")+" FROM DS_user WHERE DS_sAMAccountName = "+c.fun.repair(l.login))),e=new Enumerator(response);!e.atEnd();){b=e.item();e.moveNext();l.name=b.DS_displayName;l.home=b.DS_homeDirectory;if(f=b.DS_objectSid)if(a=c.fun.bin2sid(f.value))l.sid=a;if(a=c.fun.clear(b.DS_co))d["USR-COUNTRY"]=
...
a;break}if(l.account&&!("home"in l))for(response=t.execQuery(c.fun.debug("SELECT homeDirectory FROM Win32_NetworkLoginProfile WHERE name = "+c.fun.repair(l.account))),e=new Enumerator(response);!e.atEnd();){b=e.item();e.moveNext();l.home=b.homeDirectory;break}if(l.sid&&!("profile"in l))for(response=t.execQuery(c.fun.debug("SELECT localPath FROM Win32_UserProfile WHERE sid = "+c.fun.repair(l.sid))),e=new Enumerator(response);!e.atEnd();){b=e.item();e.moveNext();l.profile=b.localPath;break}if(a=c.fun.clear(l.sid))d["USR-SID"]=
a;if(a=c.fun.clear(l.account))d["USR-ACCOUNT"]=a;if(a=c.fun.clear(l.login))d["USR-LOGIN"]=a;if(a=c.fun.clear(l.domain))d["USR-DOMAIN"]=a;if(a=c.fun.clear(l.profile))d["USR-PROFILE"]=a;if(a=c.fun.clear(l.home))d["USR-HOME"]=a;if(a=c.fun.clear(l.name))d["USR-NAME"]=a;if(a=c.fun.clear(l.name)){f=a.split(c.val.argDelim);for(h=f.length-1;-1<h;h--)a=c.fun.clear(f[h],/[\[\(,\.\)\]]/g),(k=(k=3>a.length)||c.lib.hasValue(["von"],a,!0))?f.splice(h,1):f[h]=a;if(a=f[0])d["USR-NAME-FIRST"]=a;if(a=f[1])d["USR-NAME-SECOND"]=
a;if(a=f[2])d["USR-NAME-THIRD"]=a;if(a=f[3])d["USR-NAME-FOURTH"]=a}if(m&&u&&p)for(response=u.execQuery(c.fun.debug("SELECT DS_distinguishedName FROM DS_computer WHERE DS_cn = "+c.fun.repair(m))),e=new Enumerator(response);!e.atEnd();){b=e.item();e.moveNext();if(a=c.fun.clear(b.DS_distinguishedName))d["NET-HOST-DN"]=a;break}u=0;response=t.execQuery(c.fun.debug("SELECT architecture, maxClockSpeed, name, revision, numberOfCores, socketDesignation FROM Win32_Processor WHERE role = 'CPU'"));for(e=new Enumerator(response);!e.atEnd();){b=
e.item();e.moveNext();0==b.architecture?d["CPU-ARCHITECTURE"]="x86":9==b.architecture&&(d["CPU-ARCHITECTURE"]="x64");if(a=b.maxClockSpeed)d["CPU-SPEED"]=c.fun.info2str(1E6*a,2,1E3)+"Hz";if(a=b.maxClockSpeed)d["CPU-SPEED-VAL"]=1E6*a;if(a=c.fun.clear(b.name,"CPU","APU","Процессор","Processor","with","Radeon HD Graphics","11th Gen"))d["CPU-NAME"]=a;if(a=c.fun.clear(b.revision))d["CPU-VERSION"]=a;if(a=c.fun.clear(b.numberOfCores))d["CPU-CORE"]=a;if(a=c.fun.clear(b.socketDesignation,"SOCKET 0"))d["CPU-SOCKET"]=
a;if(a=b.maxClockSpeed)u+=2.26143*Math.sqrt(a/1E3);if(a=b.numberOfCores)u*=1.02033*Math.sqrt(a);break}u&&(w=w?Math.min(w,u):u);response=t.execQuery(c.fun.debug("SELECT level, maxCacheSize FROM Win32_CacheMemory"));for(e=new Enumerator(response);!e.atEnd();)if(b=e.item(),e.moveNext(),a=b.maxCacheSize)d["CPU-CACHE-L"+(b.level-2)]=c.fun.info2str(1024*a,0)+"B";k=u=0;response=t.execQuery(c.fun.debug("SELECT capacity, speed FROM Win32_PhysicalMemory"));for(e=new Enumerator(response);!e.atEnd();){b=e.item();
e.moveNext();if(a=b.capacity)d["RAM-SIZE-VAL"]=k+=1*a;if(a=b.capacity)d["RAM-SIZE"]=c.fun.info2str(k,0)+"B";if(a=b.speed)d["RAM-SPEED"]=a+" MHz";if(a=b.speed)d["RAM-SPEED-VAL"]=1E6*a;if(a=k)u=2.51143*Math.sqrt(a/1024/1024/1024);if(a=b.speed)u*=.92245*Math.sqrt(a/1E3)}u&&(w=w?Math.min(w,u):u);g=f=null;response=t.execQuery(c.fun.debug("SELECT adapterRam, name, driverVersion, currentHorizontalResolution, currentRefreshRate, currentBitsPerPixel, currentVerticalResolution FROM Win32_VideoController"));
for(e=new Enumerator(response);!e.atEnd();)b=e.item(),e.moveNext(),b.currentHorizontalResolution&&b.currentVerticalResolution&&(k="Microsoft Remote Display",(a=g?0:1)||(a=b.name&&-1!=b.name.indexOf(k)?1:a),a||(a=g.name&&-1!=g.name.indexOf(k)?-1:a),a||(a=c.lib.compare(b.currentHorizontalResolution,g.currentHorizontalResolution)),a||(a=c.lib.compare(b.currentVerticalResolution,g.currentVerticalResolution)),0<a&&(g=b)),(a=f?0:1)||(a=c.lib.compare(Math.abs(b.adapterRam),Math.abs(f.adapterRam))),a||(a=
c.lib.compare(b.currentHorizontalResolution,f.currentHorizontalResolution)),a||(a=c.lib.compare(b.currentVerticalResolution,f.currentVerticalResolution)),0<a&&(f=b);if(b=f){if(a=b.adapterRam)d["GPU-SIZE"]=c.fun.info2str(Math.abs(a),0)+"B";if(a=b.adapterRam)d["GPU-SIZE-VAL"]=Math.abs(a);if(a=c.fun.clear(b.name,"GPU","Видеоустройство","Family","Chipset","Series","Graphics","Adapter"))d["GPU-NAME"]=a;if(a=c.fun.clear(b.driverVersion))d["GPU-VERSION"]=a;if(a=g&&g.currentHorizontalResolution)d["GPU-RESOLUTION-X"]=
a;if(a=g&&g.currentVerticalResolution)d["GPU-RESOLUTION-Y"]=a;if(a=g&&g.currentRefreshRate)d["GPU-FREQUENCY"]=c.fun.info2str(a,0,1E3)+"Hz";if(a=g&&g.currentRefreshRate)d["GPU-FREQUENCY-VAL"]=a;if(a=g&&g.currentBitsPerPixel)d["GPU-COLOR"]=c.fun.info2str(a,0)+"bit";if(a=g&&g.currentBitsPerPixel)d["GPU-COLOR-VAL"]=a;g&&g.currentHorizontalResolution&&g.currentVerticalResolution&&(d["GPU-RESOLUTION"]=g.currentHorizontalResolution+" x "+g.currentVerticalResolution)}if(z){g=null;response=z.execQuery(c.fun.debug("SELECT instanceName, serialNumberId, userFriendlyName, userFriendlyNameLength, weekOfManufacture, yearOfManufacture FROM WmiMonitorID WHERE active = TRUE"));
for(e=new Enumerator(response);!e.atEnd();)if(b=e.item(),e.moveNext(),b.userFriendlyNameLength){b.instanceName&&(g=b);if(a=c.fun.bin2str(b.userFriendlyName))if(a=c.fun.clear(a," 4K"," HDR"))d["MON-NAME"]=a;(a=c.fun.bin2str(b.serialNumberId))&&(a=c.fun.clear(a))&&"0"!=a&&(d["MON-SERIAL"]=a);b.yearOfManufacture&&b.weekOfManufacture&&(d["MON-RELEASE"]=c.lib.date2str(c.fun.getDateOfWeek(b.weekOfManufacture,b.yearOfManufacture),"d.m.Y H:i:s"));b.yearOfManufacture&&b.weekOfManufacture&&(d["MON-RELEASE-DATE"]=
c.lib.date2str(c.fun.getDateOfWeek(b.weekOfManufacture,b.yearOfManufacture),"d.m.Y"));break}if(g&&g.instanceName)for(response=z.execQuery(c.fun.debug("SELECT maxHorizontalImageSize, maxVerticalImageSize FROM WmiMonitorBasicDisplayParams WHERE instanceName = "+c.fun.repair(g.instanceName))),e=new Enumerator(response);!e.atEnd();){b=e.item();e.moveNext();if(a=b.maxHorizontalImageSize)d["MON-SIZE-X"]=a;if(a=b.maxVerticalImageSize)d["MON-SIZE-Y"]=a;b.maxHorizontalImageSize&&b.maxVerticalImageSize&&(a=
Math.sqrt(Math.pow(b.maxHorizontalImageSize,2)+Math.pow(b.maxVerticalImageSize,2)),a=Math.round(a/2.54),d["MON-SIZE"]=b.maxHorizontalImageSize+" x "+b.maxVerticalImageSize+" cm / "+a+" in",d["MON-SIZE-Z"]=a);break}}u=0;response=C?C.execQuery(c.fun.debug("SELECT model, firmwareVersion, mediaType, serialNumber, size FROM MSFT_PhysicalDisk")):t.execQuery(c.fun.debug("SELECT * FROM Win32_DiskDrive"));for(e=new Enumerator(response);!e.atEnd();){b=e.item();e.moveNext();switch(b.mediaType){case 0:k="USB";
break;case 3:k="HDD";break;case 4:k="SSD";break;case 5:k="SCM";break;case "Removable Media":k="USB";break;case "Fixed\thard disk media":case "Fixed hard disk media":k=b.model&&-1!=b.model.indexOf("Solid")?"SSD":b.model&&-1!=b.model.indexOf("SSD")?"SSD":b.model&&!b.model.indexOf("ADATA")?"SSD":"HDD";break;default:k=""}if((!b.model||-1==b.model.indexOf("Raid"))&&k&&!d[k+"-NAME"]){if(a=c.fun.clear(b.model,"ATA Device","SCSI Disk Device","USB Device","SSD","SATA"))d[k+"-NAME"]=a;if(a=c.fun.clear(b.firmwareVersion||
b.firmwareRevision))d[k+"-VERSION"]=a;if(a=c.fun.clear(b.serialNumber))d[k+"-SERIAL"]=a;if(a=b.size)d[k+"-SIZE"]=c.fun.info2str(a,0)+"B";if(a=b.size)d[k+"-SIZE-VAL"]=a;k&&(u=Math.max(u,"SDD"==k?15.51143:7.14577))}}u&&(w=w?Math.min(w,u):u);response=t.execQuery(c.fun.debug("SELECT mediaType, caption, drive FROM Win32_CDROMDrive"));for(e=new Enumerator(response);!e.atEnd();)if(b=e.item(),e.moveNext(),!b.caption||-1==b.caption.indexOf("Alcohol"))if(!b.caption||-1==b.caption.indexOf("Virtual"))if(!b.caption||
