...
// 0.1.5-beta.2 библиотека функций общего назначения
...
// 0.1.3-beta.2 функци для работы в среде windows script host
(function(q,t,u){q.wsh={getLDAP:function(b,c,e,a,g){var m=[],n=[],d=0;if(!d){var f=new ActiveXObject("ADODB.Connection");var r=new ActiveXObject("NameTranslate");var h=new ActiveXObject("ADODB.Command")}d||(f.provider="ADsDSOObject",a&&(f.properties("User ID").value=a),g&&(f.properties("Password").value=g),f.open("Active Directory Provider"),h.activeConnection=f,h.properties("Searchscope").value=2,h.properties("Page Size").value=100);if(!d)if(e)var l=e;else e=GetObject("LDAP://RootDSE"),l=e.get("DefaultNamingContext");
d||(n=[b],c&&n.push(c));for(g=n.length-1;!d&&-1<g;g--){b=n[g];c=e=null;f="SELECT distinguishedName FROM 'LDAP://"+l+"'";if(!d&&b&&!c&&q.lib.validate(b,"guid")){a=(""+b).toUpperCase();try{r.init(3,""),r.set(7,a),c=r.get(1)}catch(p){d=1}}if(!d&&b&&!c)try{(c=b.get("distinguishedName"))&&(e=b)}catch(p){}if(!d&&b&&!c){a=f+" WHERE name = '"+b+"' OR sAMAccountName = '"+b+"' OR distinguishedName = '"+b+"'";try{h.commandText=a;var k=h.execute();k.recordCount&&(c=k.fields("distinguishedName").value)}catch(p){}}if(!d&&
!c){b?(a=""+b,a=q.lib.template(a,{protocol:"LDAP:",parent:l,select:f}),a.toUpperCase().indexOf("WHERE")?a.toUpperCase().indexOf("SELECT")&&(d=2):a=f+" "+a):a=g?f+" WHERE distinguishedName = '"+l+"'":f;try{h.commandText=a,k=h.execute(),k.recordCount||(d=4)}catch(p){d=3}}if(!d&&c&&!g)if(e)m.push(e);else{a=c;try{e=GetObject("LDAP://"+a),m.push(e)}catch(p){d=5}}if(!d&&!c)for(b=0,f=k.recordCount;b<f;k.moveNext(),b++)a=k.fields("distinguishedName").value,b||(c=a),g||(e=GetObject("LDAP://"+a),m.push(e));
d||(l=c)}return m},iconv:function(b,c,e){var a=new ActiveXObject("ADODB.Stream");a.type=2;a.mode=3;a.open();a.charset=c;a.writeText(e);a.position=0;a.charset=b;e=a.readText();a.close();return e}}})(App.prototype,WSH);

// 0.1.0 расширяем глобальные функции среды
function setEnv(env, key, value) { env(key) = value; }

// 0.1.4-beta.6 определяет дополнительные переменные среды
var env=new App({aptPref:"apt",aptLen:3,aptNone:"XXX",wsPref:"c",wsLen:1,wsFirstDesc:"Основной",wsNextDesc:"Дополнительный",wsNoneDesc:"Временный",runStyle:1,defReturn:0,driveMinSize:27917287424,argWrap:'"',argDelim:" ",linDelim:"\r\n",keyDelim:"\\",chrDelim:"@",getDelim:"+",setDelim:"#",iniDelim:"=",csvDelim:";",tsvDelim:"\t",envType:"Process"});
(function(w,c,J){c.lib.extend(c,{fun:{wql2date:function(b,p){return new Date(b?Date.UTC(1*b.substr(0,4),1*b.substr(4,2)-1,1*b.substr(6,2),1*b.substr(8,2),1*b.substr(10,2)-1*b.substr(21,4)+(p||0),1*b.substr(12,2),1*b.substr(14,3)):0)},bin2key:function(b){var p="";var r=b.toArray();b=Math.floor(r[66]/6)&1;r[66]=r[66]&247|4*(b&2);for(var d=24;-1<d;d--){var l=0;for(var q=14;-1<q;q--)l*=256,l=r[q+52]+l,r[q+52]=Math.floor(l/24),l%=24;p="BCDFGHJKMPQRTVWXY2346789".substr(l,1)+p}1==b&&(b=p.substr(1,l),p=p.substr(1).replace(b,
b+"N"));return[p.substr(0,5),p.substr(5,5),p.substr(10,5),p.substr(15,5),p.substr(20,5)].join("-")},info2str:function(b,p,r){if(!r||2>r)r=1024;if(!p||0>p)p=0;var d=Math.pow(10,p);for(var l=-1;b>=r;l++)b/=r;b=Math.ceil(b*d)/d;b=c.lib.num2str(b,-1<l?p:0,",","");return b+=" "+"КМГТПЭЗЙ".charAt(l)},clear:function(b){b=b?""+b:"";"INVALID"==b&&(b="");"To be filled by O.E.M."==b&&(b="");"System Product Name"==b&&(b="");"System Serial Number"==b&&(b="");"System manufacturer"==b&&(b="");"Default string"==
b&&(b="");"empty"==b&&(b="");"None"==b&&(b="");for(var p=1,r=arguments.length;p<r;p++)b=b.replace(arguments[p],"");return b.replace(/\(R\)/gi,"").replace(/\(Registered Trademark\)/gi,"").replace(/\(Microsoft Corporation\)/gi,"").replace(/\(Корпорация Майкрософт\)/gi,"").replace(/\(Майкрософт\)/gi,"").replace(/\(TM\)/gi,"").replace(/^[/'"]|[/"']$/g,"").replace(/^\s+|\s+$/g,"").replace(/\.+$/,"").replace(/\s(?=\s)/gi,"")},repair:function(b){return("'"+(b?b:"")+"'").replace(/\\/g,"\\\\")}},init:function(){var b,
p="",r="";var d={};var l={},q=0;var v=new Date;for(b=0;b<w.arguments.length;b++){var a=w.arguments.item(b);if(!("context"in l)){var e=a.split(c.val.keyDelim);if(3==e.length&&!e[0]&&!e[1]){l.context=e[2];continue}}if(!("input"in l)&&(e=a.split(c.val.chrDelim),2==e.length&&c.lib.hasValue(["ini","csv","tsv","CSV","TSV"],e[0],!0)&&e[1])){l.input=e[0];l.charset=e[1];continue}if(!("output"in l)&&(e=a.split(c.val.chrDelim),1==e.length&&c.lib.hasValue(["ini","csv","tsv","CSV","TSV"],e[0],!0))){l.output=e[0];
continue}if("silent"in l||"silent"!=a)if("nowait"in l||"nowait"!=a)break;else l.nowait=!0;else l.silent=!0}var D=b;"context"in l||(l.context=".");"auto"==l.charset&&(l.charset="windows-1251");var E=new ActiveXObject("WScript.Shell");var y=new ActiveXObject("wbemScripting.Swbemlocator");y.security_.impersonationLevel=3;if(l.context)try{var n=y.connectServer(l.context,"root\\CIMV2");var G=y.connectServer(l.context,"root\\directory\\LDAP");var t=y.connectServer(l.context,"root\\Microsoft\\Windows\\Storage");
var m=y.connectServer(l.context,"root\\default").get("stdRegProv")}catch(B){n=null}if(n){var f=m.methods_.item("getBinaryValue");b=f.inParameters.spawnInstance_();b.hDefKey=2147483650;b.sSubKeyName="SOFTWARE\\Microsoft\\Windows NT\\CurrentVersion";b.sValueName="DigitalProductId";b=m.execMethod_(f.name,b);b.returnValue||(a=c.fun.bin2key(b.uValue))&&"BBBBB-BBBBB-BBBBB-BBBBB-BBBBB"!=a&&(d["SYS-KEY"]=a);response=n.execQuery("SELECT * FROM Win32_OperatingSystem WHERE primary = TRUE");for(f=new Enumerator(response);!f.atEnd();){b=
f.item();f.moveNext();if(a=b.systemDrive)var H=a;if(a=b.localDateTime)v=c.fun.wql2date(a);if(a=c.fun.clear(b.caption,"Майкрософт","Microsoft","Edition","x64",","))d["SYS-NAME"]=a;if(a=c.fun.clear(b.version))d["SYS-VERSION"]=a;if(a=b.localDateTime)d["SYS-TIME"]=c.lib.date2str(c.fun.wql2date(a),"d.m.Y H:i:s");if(a=b.localDateTime)d["SYS-TIME-DATE"]=c.lib.date2str(c.fun.wql2date(a),"d.m.Y");if(a=c.fun.clear(b.systemDrive))d["SYS-DRIVE"]=a;if(a=b.installDate)d["SYS-INSTALL"]=c.lib.date2str(c.fun.wql2date(a),
"d.m.Y H:i:s");if(a=b.installDate)d["SYS-INSTALL-DATE"]=c.lib.date2str(c.fun.wql2date(a),"d.m.Y");if(a=b.lastBootUpTime)d["SYS-RESET"]=c.lib.date2str(c.fun.wql2date(a),"d.m.Y H:i:s");if(a=b.lastBootUpTime)d["SYS-RESET-DATE"]=c.lib.date2str(c.fun.wql2date(a),"d.m.Y");if(a=c.fun.clear(b.serialNumber))d["SYS-SERIAL"]=a;if(a=c.fun.clear(b.description))d["SYS-DESCRIPTION"]=a;d["SYS-ARCHITECTURE"]=b.osArchitecture&&!b.osArchitecture.indexOf("64")?"x64":"x86";break}response=n.execQuery("SELECT manufacturer, product, serialNumber FROM Win32_BaseBoard WHERE hostingBoard = TRUE");
for(f=new Enumerator(response);!f.atEnd();){b=f.item();f.moveNext();if(a=c.fun.clear(b.product))d["PCB-NAME"]=a;a&&(a=c.fun.clear(b.manufacturer,"Inc.").replace("Hewlett-Packard","HP"))&&(d["PCB-NAME"]=a.split(" ")[0]+" "+c.fun.clear(b.product));if(a=c.fun.clear(b.serialNumber))d["PCB-SERIAL"]=a;break}response=n.execQuery("SELECT releaseDate, manufacturer, smBIOSBIOSVersion, serialNumber FROM Win32_BIOS WHERE primaryBIOS = TRUE");for(f=new Enumerator(response);!f.atEnd();){b=f.item();f.moveNext();
if(a=c.fun.clear(b.releaseDate))d["PCB-BIOS-RELEASE"]=c.lib.date2str(c.fun.wql2date(a,v.getTimezoneOffset()),"d.m.Y H:i:s");if(a=c.fun.clear(b.releaseDate))d["PCB-BIOS-RELEASE-DATE"]=c.lib.date2str(c.fun.wql2date(a,v.getTimezoneOffset()),"d.m.Y");if(a=c.fun.clear(b.manufacturer,"Inc."))d["PCB-BIOS-MANUFACTURE"]=a;if(a=c.fun.clear(b.smBIOSBIOSVersion))d["PCB-BIOS-VERSION"]=a;if(a=c.fun.clear(b.serialNumber))d["PCB-BIOS-SERIAL"]=a;break}var u="";response=n.execQuery("SELECT * FROM Win32_NetworkAdapterConfiguration WHERE ipEnabled = TRUE");
for(f=new Enumerator(response);!f.atEnd();){b=f.item();f.moveNext();if(a=b.interfaceIndex)u=a;if(null!=b.ipAddress){e=b.ipAddress.toArray();var g=0;for(var k=e.length;g<k;g++)(a=c.fun.clear(e[g]))&&-1!=a.indexOf(".")&&!d["NET-IP-V4"]&&(d["NET-IP-V4"]=a),a&&-1==a.indexOf(".")&&!d["NET-IP-V6"]&&(d["NET-IP-V6"]=a)}if(null!=b.defaultIPGateway)for(e=b.defaultIPGateway.toArray(),g=0,k=e.length;g<k;g++)(a=c.fun.clear(e[g]))&&-1!=a.indexOf(".")&&!d["NET-GATEWAY-V4"]&&(d["NET-GATEWAY-V4"]=a),a&&-1==a.indexOf(".")&&
!d["NET-GATEWAY-V6"]&&(d["NET-GATEWAY-V6"]=a);if(null!=b.dnsServerSearchOrder)for(e=b.dnsServerSearchOrder.toArray(),g=0,k=e.length;g<k;g++)(a=c.fun.clear(e[g]))&&-1!=a.indexOf(".")&&!d["NET-DNS-V4"]&&(d["NET-DNS-V4"]=a),a&&-1==a.indexOf(".")&&!d["NET-DNS-V6"]&&(d["NET-DNS-V6"]=a);if(null!=b.ipSubnet)for(e=b.ipSubnet.toArray(),g=0,k=e.length;g<k;g++)(a=c.fun.clear(e[g]))&&-1!=a.indexOf(".")&&!d["NET-SUBNET-V4"]&&(d["NET-SUBNET-V4"]=a),a&&-1==a.indexOf(".")&&!d["NET-SUBNET-V6"]&&(d["NET-SUBNET-V6"]=
a);if(a=c.fun.clear(b.dhcpServer))d["NET-DHCP-V4"]=a;if(a=c.fun.clear(b.description,"Сетевой адаптер","Адаптер","для виртуальной сети","Сетевая карта","Контроллер","NIC (NDIS 6.20)","- Минипорт планировщика пакетов","Family Controller","Adapter"))d["NET-NAME"]=a;if(a=c.fun.clear(b.macAddress))d["NET-MAC"]=a;break}e=0;response=n.execQuery("SELECT speed, timeOfLastReset FROM Win32_NetworkAdapter WHERE netEnabled = TRUE AND interfaceIndex = "+c.fun.repair(u));for(f=new Enumerator(response);!f.atEnd();){b=
f.item();f.moveNext();if(a=b.speed)d["NET-SPEED"]=c.fun.info2str(a,0,1E3)+"бит/с";if(a=b.speed)d["NET-SPEED-VAL"]=a;if(a=b.timeOfLastReset)d["NET-RESET"]=c.lib.date2str(c.fun.wql2date(a),"d.m.Y H:i:s");if(a=b.timeOfLastReset)d["NET-RESET-DATE"]=c.lib.date2str(c.fun.wql2date(a),"d.m.Y");if(a=b.speed)e+=8.12567*Math.sqrt(a/100/1E3/1E3);break}e&&(q=q?Math.min(q,e):e);response=n.execQuery("SELECT * FROM Win32_ComputerSystem");for(f=new Enumerator(response);!f.atEnd();){b=f.item();f.moveNext();if(a=b.dnsHostName)p=
a;if(a=b.name)p||(p=a.toLowerCase());b.domain!=b.workgroup&&(r=b.domain);if(a=b.userName)var A=a.split(c.val.keyDelim)[0];if(a=b.userName)var z=a.split(c.val.keyDelim)[1];if(a=b.userName)var x=a;if(a=c.fun.clear(p))d["NET-HOST"]=a;if(a=c.fun.clear(b.domain))d["NET-DOMAIN"]=a;if(a=c.fun.clear(b.model))d["DEV-NAME"]=a;a&&(a=c.fun.clear(b.manufacturer,"Inc.","Hewlett-Packard"))&&(d["DEV-NAME"]=a.split(" ")[0]+" "+c.fun.clear(b.model));break}x||(e=[],f=m.methods_.item("getStringValue"),b=f.inParameters.spawnInstance_(),
b.hDefKey=2147483650,b.sSubKeyName="SOFTWARE\\Microsoft\\Windows NT\\CurrentVersion\\Winlogon",b.sValueName="DefaultDomainName",b=m.execMethod_(f.name,b),!b.returnValue&&b.sValue&&e.push(b.sValue),f=m.methods_.item("getStringValue"),b=f.inParameters.spawnInstance_(),b.hDefKey=2147483650,b.sSubKeyName="SOFTWARE\\Microsoft\\Windows NT\\CurrentVersion\\Winlogon",b.sValueName="DefaultUserName",b=m.execMethod_(f.name,b),!b.returnValue&&b.sValue&&e.push(b.sValue),2==e.length&&(A=e[0]),2==e.length&&(z=e[1]),
2==e.length&&(x=e.join(c.val.keyDelim)));if(!x){e=null;response=n.execQuery("SELECT lastUseTime, localPath, loaded FROM Win32_UserProfile WHERE special = FALSE");for(f=new Enumerator(response);!f.atEnd();)b=f.item(),f.moveNext(),(a=e?0:1)||(a=b.loaded&&!e.loaded?1:a),a||(a=c.lib.compare(c.fun.wql2date(b.lastUseTime),c.fun.wql2date(e.lastUseTime))),0<a&&(e=b);if(b=e){if(a=b.localPath.split(c.val.keyDelim).pop())z=a;if(a=b.localPath)var F=a}}if(z){var h=n;g=[".",r];do{response=h.execQuery("SELECT domain, name, fullName, sid FROM Win32_UserAccount WHERE name = "+
c.fun.repair(z)+(A?" AND domain = "+c.fun.repair(A):""));for(f=new Enumerator(response);!f.atEnd();){b=f.item();f.moveNext();if(a=c.fun.clear(b.fullName))d["USR-NAME"]=a;if(a=c.fun.clear(b.fullName)){e=a.split(c.val.argDelim);for(k=e.length-1;-1<k;k--)a=c.fun.clear(e[k],/[\[\(,\.\)\]]/g),2<a.length?e[k]=a:e.splice(k,1);if(a=e[0])d["USR-NAME-FIRST"]=a;if(a=e[1])d["USR-NAME-SECOND"]=a;if(a=e[2])d["USR-NAME-THIRD"]=a;if(a=e[3])d["USR-NAME-FOURTH"]=a}e=[];if(a=b.sid)var C=a;(a=b.domain)&&e.push(a);(a=
b.name)&&e.push(a);2==e.length&&(A=e[0]);2==e.length&&(z=e[1]);2==e.length&&(x=e.join(c.val.keyDelim));break}try{h=!g.length||C?null:y.connectServer(g.shift(),"root\\CIMV2")}catch(B){h=null}}while(h)}if(x)for(response=n.execQuery("SELECT homeDirectory FROM Win32_NetworkLoginProfile WHERE name = "+c.fun.repair(x)),f=new Enumerator(response);!f.atEnd();){b=f.item();f.moveNext();if(a=b.homeDirectory)var I=a;break}if(!F&&C)for(response=n.execQuery("SELECT localPath FROM Win32_UserProfile WHERE sid = "+
c.fun.repair(C)),f=new Enumerator(response);!f.atEnd();){b=f.item();f.moveNext();if(a=b.localPath)F=a;break}if(a=c.fun.clear(C))d["USR-SID"]=a;if(a=c.fun.clear(x))d["USR-LOGIN"]=a;if(a=c.fun.clear(A))d["USR-DOMAIN"]=a;if(a=c.fun.clear(z))d["USR-ACCOUNT"]=a;if(a=c.fun.clear(F))d["USR-PROFILE"]=a;if(a=c.fun.clear(I))d["USR-HOME"]=a;if(p)for(response=G.execQuery("SELECT DS_distinguishedName FROM DS_computer WHERE DS_cn = "+c.fun.repair(p)),f=new Enumerator(response);!f.atEnd();){b=f.item();f.moveNext();
if(a=c.fun.clear(b.DS_distinguishedName))d["NET-HOST-DN"]=a;break}e=0;response=n.execQuery("SELECT architecture, maxClockSpeed, name, revision, numberOfCores, socketDesignation FROM Win32_Processor WHERE role = 'CPU'");for(f=new Enumerator(response);!f.atEnd();){b=f.item();f.moveNext();0==b.architecture?d["CPU-ARCHITECTURE"]="x86":9==b.architecture&&(d["CPU-ARCHITECTURE"]="x64");if(a=b.maxClockSpeed)d["CPU-SPEED"]=c.fun.info2str(1E6*a,2,1E3)+"Гц";if(a=b.maxClockSpeed)d["CPU-SPEED-VAL"]=1E6*a;if(a=
c.fun.clear(b.name,"CPU","APU","Процессор","Processor","with","Radeon HD Graphics"))d["CPU-NAME"]=a;if(a=c.fun.clear(b.revision))d["CPU-VERSION"]=a;if(a=b.numberOfCores)d["CPU-CORE"]=a;if(a=c.fun.clear(b.socketDesignation,"SOCKET 0"))d["CPU-SOCKET"]=a;if(a=b.maxClockSpeed)e+=2.26143*Math.sqrt(a/1E3);if(a=b.numberOfCores)e*=1.02033*Math.sqrt(a);break}e&&(q=q?Math.min(q,e):e);response=n.execQuery("SELECT level, maxCacheSize FROM Win32_CacheMemory");for(f=new Enumerator(response);!f.atEnd();)if(b=f.item(),
f.moveNext(),a=b.maxCacheSize)d["CPU-CACHE-L"+(b.level-2)]=c.fun.info2str(1024*a,0)+"Б";h=e=0;response=n.execQuery("SELECT capacity, speed FROM Win32_PhysicalMemory");for(f=new Enumerator(response);!f.atEnd();){b=f.item();f.moveNext();if(a=b.capacity)d["RAM-SIZE-VAL"]=h+=1*a;b.capacity&&(d["RAM-SIZE"]=c.fun.info2str(h,0)+"Б");if(a=b.speed)d["RAM-SPEED"]=a+" МГц";if(a=b.speed)d["RAM-SPEED-VAL"]=1E6*a;if(a=h)e=2.51143*Math.sqrt(a/1024/1024/1024);if(a=b.speed)e*=.92245*Math.sqrt(a/1E3)}e&&(q=q?Math.min(q,
e):e);response=n.execQuery("SELECT adapterRam, name, driverVersion, currentHorizontalResolution, currentRefreshRate, currentBitsPerPixel, currentVerticalResolution FROM Win32_VideoController");for(f=new Enumerator(response);!f.atEnd();){b=f.item();f.moveNext();if(a=b.adapterRam)d["GPU-SIZE"]=c.fun.info2str(Math.abs(a),0)+"Б";if(a=b.adapterRam)d["GPU-SIZE-VAL"]=Math.abs(a);if(a=c.fun.clear(b.name,"GPU","Видеоустройство","Family","Chipset","Series","Graphics","Adapter"))d["GPU-NAME"]=a;if(a=c.fun.clear(b.driverVersion))d["GPU-VERSION"]=
a;b.currentHorizontalResolution&&b.currentVerticalResolution&&(d["GPU-RESOLUTION"]=b.currentHorizontalResolution+" x "+b.currentVerticalResolution);if(a=b.currentHorizontalResolution)d["GPU-RESOLUTION-X"]=a;if(a=b.currentVerticalResolution)d["GPU-RESOLUTION-Y"]=a;if(a=b.currentRefreshRate)d["GPU-FREQUENCY"]=c.fun.info2str(a,0,1E3)+"Гц";if(a=b.currentRefreshRate)d["GPU-FREQUENCY-VAL"]=a;if(a=b.currentBitsPerPixel)d["GPU-COLOR"]=c.fun.info2str(a,0)+"бит"+c.lib.numDeclin(a,"","","а");if(a=b.currentBitsPerPixel)d["GPU-COLOR-VAL"]=
a;break}e=0;response=t.execQuery("SELECT friendlyName, firmwareVersion, mediaType, serialNumber, size FROM MSFT_PhysicalDisk");for(f=new Enumerator(response);!f.atEnd();){b=f.item();f.moveNext();switch(b.mediaType){case 0:h="USB";break;case 3:h="HDD";break;case 4:h="SSD";break;case 5:h="SCM";break;default:h=""}if((!b.friendlyName||-1==b.friendlyName.indexOf("Raid"))&&h&&!d[h+"-NAME"]){if(a=c.fun.clear(b.friendlyName,"ATA Device","SCSI Disk Device","USB Device","SSD","SATA"))d[h+"-NAME"]=a;if(a=c.fun.clear(b.firmwareVersion))d[h+
"-VERSION"]=a;if(a=c.fun.clear(b.serialNumber))d[h+"-SERIAL"]=a;if(a=b.size)d[h+"-SIZE"]=c.fun.info2str(a,0)+"Б";if(a=b.size)d[h+"-SIZE-VAL"]=a;h&&(e=Math.max(e,"SDD"==h?15.51143:7.14577))}}e&&(q=q?Math.min(q,e):e);response=n.execQuery("SELECT mediaType, caption, drive FROM Win32_CDROMDrive");for(f=new Enumerator(response);!f.atEnd();)if(b=f.item(),f.moveNext(),!b.caption||-1==b.caption.indexOf("Alcohol"))if(!b.caption||-1==b.caption.indexOf("Virtual")){switch(b.mediaType){case "CD-ROM":d["ROM-TYPE"]=
"CD";break;case "DVD-ROM":d["ROM-TYPE"]="DVD";break;case "CD Writer":d["ROM-TYPE"]="CD-RW";break;case "DVD Writer":d["ROM-TYPE"]="DVD-RW"}if(a=c.fun.clear(b.caption,"ATA Device","SCSI CdRom Device"))d["ROM-NAME"]=a;if(a=c.fun.clear(b.drive))d["ROM-DRIVE"]=a;break}response=n.execQuery("SELECT caption, size FROM Win32_LogicalDisk WHERE driveType = 2 OR driveType = 3 OR driveType = 4");for(f=new Enumerator(response);!f.atEnd();)b=f.item(),f.moveNext(),b.caption&&-1!=b.caption.indexOf(H)||d["BAK-DRIVE"]||
(a=c.fun.clear(b.caption))&&b.size>=c.val.driveMinSize&&(d["BAK-DRIVE"]=a);u="";h="DisplayIcon";e=["SOFTWARE\\Microsoft\\Windows\\CurrentVersion\\Uninstall\\F3 TAIL","SOFTWARE\\Wow6432Node\\Microsoft\\Windows\\CurrentVersion\\Uninstall\\F3 TAIL","SOFTWARE\\Microsoft\\Windows\\CurrentVersion\\Uninstall\\еФарма","SOFTWARE\\Wow6432Node\\Microsoft\\Windows\\CurrentVersion\\Uninstall\\еФарма"];a="";f=m.methods_.item("getStringValue");g=0;for(k=e.length;g<k&&!a;g++)b=f.inParameters.spawnInstance_(),b.hDefKey=
2147483650,b.sSubKeyName=e[g],b.sValueName=h,b=m.execMethod_(f.name,b),!b.returnValue&&b.sValue&&(a=c.fun.clear(b.sValue));a&&(e=a.split(c.val.keyDelim),e.pop(),e.pop(),u=e.join(c.val.keyDelim),d["APP-EFARMA-DIR"]=u);h="\\Client\\ePlus.Client.exe";e=[u];a="";g=0;for(k=e.length;g<k;g++)g&&(a+=" OR "),a+="name = "+c.fun.repair(e[g]+h);response=n.execQuery("SELECT name FROM CIM_DataFile WHERE "+a);for(f=new Enumerator(response);!f.atEnd();)for(b=f.item(),f.moveNext(),g=0,k=e.length;g<k;g++)if(a=e[g]+
h,b.name&&b.name.toLowerCase()==a.toLowerCase()){d["APP-EFARMA-CLIENT"]=a;break}h="\\ARM\\ePlus.ARMCasherNew.exe";e=[u];a="";g=0;for(k=e.length;g<k;g++)g&&(a+=" OR "),a+="name = "+c.fun.repair(e[g]+h);response=n.execQuery("SELECT name FROM CIM_DataFile WHERE "+a);for(f=new Enumerator(response);!f.atEnd();)for(b=f.item(),f.moveNext(),g=0,k=e.length;g<k;g++)if(a=e[g]+h,b.name&&b.name.toLowerCase()==a.toLowerCase()){d["APP-EFARMA-CASHER"]=a;break}h="\\UpdateServer\\ePlus.UpdateServer.exe";e=[u];a="";
g=0;for(k=e.length;g<k;g++)g&&(a+=" OR "),a+="name = "+c.fun.repair(e[g]+h);response=n.execQuery("SELECT name FROM CIM_DataFile WHERE "+a);for(f=new Enumerator(response);!f.atEnd();)for(b=f.item(),f.moveNext(),g=0,k=e.length;g<k;g++)if(a=e[g]+h,b.name&&b.name.toLowerCase()==a.toLowerCase()){d["APP-EFARMA-UPDATER"]=a;break}h="lic";e=[u+"\\UpdateServer\\",u+"\\Client\\",u+"\\ARM\\"];a="";g=0;for(k=e.length;g<k;g++)g&&(a+=" OR "),a+="drive = "+c.fun.repair(c.lib.strim(e[g],"",":",!0,!1))+" ",a+="AND path = "+
c.fun.repair(c.lib.strim(e[g],":","",!1,!1))+" ",a+="AND extension = "+c.fun.repair(h);response=n.execQuery("SELECT name, fileName FROM CIM_DataFile WHERE "+a);for(f=new Enumerator(response);!f.atEnd();)for(b=f.item(),f.moveNext(),g=0,k=e.length;g<k;g++)if(a=e[g]+b.fileName+"."+h,b.name&&b.name.toLowerCase()==a.toLowerCase()){d["APP-EFARMA-LICENSE"]=a;break}a=c.lib.date2str(v,"Y");h="\\ULUS.exe";e=["C:\\SoftLink\\Ulus\\"+a,"C:\\LO\\ULUS\\"+a,"C:\\so\\Ulus\\"+a,"C:\\ULUS\\"+a];a="";g=0;for(k=e.length;g<
k;g++)g&&(a+=" OR "),a+="name = "+c.fun.repair(e[g]+h);response=n.execQuery("SELECT name FROM CIM_DataFile WHERE "+a);for(f=new Enumerator(response);!f.atEnd();)for(b=f.item(),f.moveNext(),g=0,k=e.length;g<k;g++)if(a=e[g]+h,b.name&&b.name.toLowerCase()==a.toLowerCase()){d["APP-ULUS"]=a;d["APP-ULUS-DIR"]=e[g];break}h="";e=["SOFTWARE\\Clients\\StartMenuInternet\\Google Chrome\\shell\\open\\command","SOFTWARE\\WOW6432Node\\Clients\\StartMenuInternet\\Google Chrome\\shell\\open\\command"];a="";f=m.methods_.item("getStringValue");
g=0;for(k=e.length;g<k&&!a;g++)b=f.inParameters.spawnInstance_(),b.hDefKey=2147483650,b.sSubKeyName=e[g],b.sValueName=h,b=m.execMethod_(f.name,b),!b.returnValue&&b.sValue&&(a=c.fun.clear(b.sValue));a&&(e=a.split(c.val.keyDelim),e.pop(),d["APP-CHROME"]=a,d["APP-CHROME-DIR"]=e.join(c.val.keyDelim));h="";e=["SOFTWARE\\VideoLAN\\VLC","SOFTWARE\\WOW6432Node\\VideoLAN\\VLC"];a="";f=m.methods_.item("getStringValue");g=0;for(k=e.length;g<k&&!a;g++)b=f.inParameters.spawnInstance_(),b.hDefKey=2147483650,b.sSubKeyName=
e[g],b.sValueName=h,b=m.execMethod_(f.name,b),!b.returnValue&&b.sValue&&(a=c.fun.clear(b.sValue));a&&(e=a.split(c.val.keyDelim),e.pop(),d["APP-VLC"]=a,d["APP-VLC-DIR"]=e.join(c.val.keyDelim));h="ClientID";e=["SOFTWARE\\TeamViewer","SOFTWARE\\WOW6432Node\\TeamViewer"];a="";f=m.methods_.item("getDWORDValue");g=0;for(k=e.length;g<k&&!a;g++)b=f.inParameters.spawnInstance_(),b.hDefKey=2147483650,b.sSubKeyName=e[g],b.sValueName=h,b=m.execMethod_(f.name,b),!b.returnValue&&b.uValue&&(a=c.fun.clear(b.uValue));
a&&(d["APP-TEAMVIEWER-ID"]=a);a=p.toLowerCase();0==a.indexOf(c.val.aptPref)&&(a=a.substr(c.val.aptPref.length,c.val.aptLen),isNaN(a)||(d["APT-NUMBER"]=a||c.val.aptNone,d["APT-NUMBER-VAL"]=Number(a)));a=p.toLowerCase();0==a.indexOf(c.val.aptPref)&&((a=a.substr(c.val.aptPref.length+c.val.aptLen))?0==a.indexOf(c.val.wsPref)?(a=a.substr(c.val.wsPref.length,c.val.wsLen),a=isNaN(a)?0:Number(a)):a=0:a=1,d["APT-COMPUTER-VAL"]=a,d["APT-COMPUTER"]=0==a?c.val.wsNoneDesc:1==a?c.val.wsFirstDesc:c.val.wsNextDesc);
e=[];d["CPU-NAME"]&&d["CPU-CORE"]&&d["CPU-SPEED"]&&e.push(c.fun.clear(d["CPU-NAME"].replace("Dual-Core","Intel"),"Dual Core","Xeon","Pentium","Celeron","Core2 Duo","Core","Processor","Athlon 64","Athlon",/,.+/,/@.+/,/\d\.d+GHz/)+" "+d["CPU-CORE"]+"x"+d["CPU-SPEED"].replace(",",".").replace(" МГц","M").replace(" ГГц","G")+"Hz");d["RAM-SIZE"]&&d["RAM-SPEED"]&&e.push(d["RAM-SIZE"].replace(" МБ","MB").replace(" ГБ","GB")+" "+d["RAM-SPEED"].replace(" МГц","M").replace(" ГГц","G")+"Hz");d["GPU-SIZE"]&&
d["GPU-NAME"]&&(-1!=d["GPU-NAME"].indexOf("GeForce")||-1!=d["GPU-NAME"].indexOf("Radeon"))&&e.push(d["GPU-SIZE"].replace(" МБ","MB").replace(" ГБ","GB")+" "+c.fun.clear(d["GPU-NAME"],"AMD","NVIDIA","GeForce","Radeon",/\(.+/));d["HDD-SIZE"]&&e.push(d["HDD-SIZE"].replace(" МБ","MB").replace(" ГБ","GB").replace(" ТБ","TB")+" HDD");d["SSD-SIZE"]&&e.push(d["SSD-SIZE"].replace(" МБ","MB").replace(" ГБ","GB").replace(" ТБ","TB")+" SSD");d["USB-SIZE"]&&e.push(d["USB-SIZE"].replace(" МБ","MB").replace(" ГБ",
"GB").replace(" ТБ","TB")+" USB");d["ROM-TYPE"]&&e.push(d["ROM-TYPE"]);e.length&&(d["DEV-DESCRIPTION"]=e.join("/"));q&&(d["DEV-BENCHMARK"]=c.lib.num2str(q,5,",",""))}n="SYS-KEY SYS-NAME SYS-VERSION SYS-TIME SYS-ARCHITECTURE SYS-DRIVE SYS-INSTALL SYS-RESET SYS-SERIAL PCB-NAME PCB-SERIAL PCB-BIOS-MANUFACTURE PCB-BIOS-RELEASE PCB-BIOS-VERSION PCB-BIOS-SERIAL NET-IP-V4 NET-GATEWAY-V4 NET-DNS-V4 NET-SUBNET-V4 NET-DHCP-V4 NET-NAME NET-MAC NET-SPEED NET-RESET NET-HOST NET-DOMAIN USR-LOGIN USR-DOMAIN USR-ACCOUNT USR-NAME USR-SID USR-PROFILE CPU-ARCHITECTURE CPU-SPEED CPU-NAME CPU-CORE CPU-SOCKET CPU-CACHE-L1 CPU-CACHE-L2 CPU-CACHE-L3 RAM-SIZE RAM-SPEED GPU-SIZE GPU-NAME GPU-VERSION GPU-RESOLUTION GPU-FREQUENCY GPU-COLOR SSD-NAME SSD-VERSION SSD-SERIAL SSD-SIZE HDD-NAME HDD-VERSION HDD-SERIAL HDD-SIZE USB-NAME USB-VERSION USB-SERIAL USB-SIZE ROM-TYPE ROM-NAME ROM-DRIVE ROM-VERSION DEV-NAME DEV-DESCRIPTION DEV-BENCHMARK".split(" ");
if(l.input)try{h="windows-1251",a=w.stdIn.readAll(),l.charset!=h&&(a=c.wsh.iconv(l.charset,h,a))}catch(B){a=""}if(l.input&&a)switch(b=0,m="",t=a.split(c.val.linDelim),l.input){case "ini":b=c.lib.ini2obj(a,!1);for(h in b)!h||!b[h]||h in d||(d[h]=b[h]);break;case "TSV":m||(m=c.val.tsvDelim);case "CSV":m||(m=c.val.csvDelim),b<t.length&&(n=t[b++].split(m));case "tsv":m||(m=c.val.tsvDelim);case "csv":for(m||(m=c.val.csvDelim);b<t.length;)for(e=t[b++].split(m),g=0,k=n.length;g<k;g++)h=n[g],!h||!e[g]||h in
d||(d[h]=e[g])}if(l.output)switch(t=[],m="",l.output){case "ini":m=c.val.argDelim+c.val.iniDelim+c.val.argDelim;e=[];for(h in d)e.push(h);e.sort();g=0;for(k=e.length;g<k;g++)h=e[g],a=d[h]?d[h]:"",a=h+m+a,t.push(a);a=t.join(c.val.linDelim);break;case "TSV":m||(m=c.val.tsvDelim);case "CSV":m||(m=c.val.csvDelim);v=!0;e=[];g=0;for(k=n.length;g<k;g++)(h=n[g])&&(v=!1),e.push(h);a=n.join(m);v||t.push(a);case "tsv":m||(m=c.val.tsvDelim);case "csv":m||(m=c.val.csvDelim);v=!0;e=[];g=0;for(k=n.length;g<k;g++)h=
n[g],(a=d[h]?d[h]:"")&&(v=!1),e.push(a);a=e.join(m);v||t.push(a);a=t.join(c.val.linDelim)}if(l.output&&a){a+=c.val.linDelim;try{w.stdOut.write(a)}catch(B){}}f=E.environment(c.val.envType);for(h in d)a=d[h],setEnv(f,h,a);f=[];for(b=D;b<w.arguments.length;b++)a=w.arguments.item(b),a=a.split(c.val.getDelim).join(c.val.setDelim),(a=E.expandEnvironmentStrings(a))&&-1==a.indexOf(c.val.argDelim)||(a=c.val.argWrap+a+c.val.argWrap),f.push(a);if(d=f.join(c.val.argDelim)){D=l.silent?0:c.val.runStyle;try{a=E.run(d,
D,!l.nowait),l.nowait&&(a=c.val.defReturn)}catch(B){a=c.val.defReturn}}else a=c.val.defReturn;w.quit(a)}})})(WSH,env);env.init();
