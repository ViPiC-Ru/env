...
e=1;e;e++)try{switch(e){case 1:var t=f.connectServer(m.context,"root\\CIMV2");break;case 2:var z=f.connectServer(m.context,"root\\WMI");break;case 3:var u=f.connectServer(".","root\\directory\\LDAP");break;case 4:var C=f.connectServer(m.context,"root\\Microsoft\\Windows\\Storage");break;case 5:var v=f.connectServer(m.context,"root\\default").get("stdRegProv");break;default:e=-1}}catch(y){switch(e){case 1:t=null;e=-1;break;case 2:z=null;break;case 3:u=null;break;case 4:C=null;break;case 5:v=null}}if(t){if(v){f=
v.methods_.item("getBinaryValue");var b=f.inParameters.spawnInstance_();b.hDefKey=2147483650;b.sSubKeyName="SOFTWARE\\Microsoft\\Windows NT\\CurrentVersion";b.sValueName=c.fun.debug("DigitalProductId");b=v.execMethod_(f.name,b);!b.returnValue&&(a=c.fun.bin2key(b.uValue),a=c.fun.clear(a))&&(d["SYS-KEY"]=a)}q="";response=t.execQuery(c.fun.debug("SELECT * FROM Win32_OperatingSystem WHERE primary = TRUE"));for(f=new Enumerator(response);!f.atEnd();){b=f.item();f.moveNext();if(a=b.systemDrive)var E=a;
...
c.lib.date2str(c.fun.wql2date(a),"d.m.Y");if(a=b.lastBootUpTime)d["SYS-RESET"]=c.lib.date2str(c.fun.wql2date(a),"d.m.Y H:i:s");if(a=b.lastBootUpTime)d["SYS-RESET-DATE"]=c.lib.date2str(c.fun.wql2date(a),"d.m.Y");if(a=c.fun.clear(b.serialNumber))d["SYS-SERIAL"]=a;if(a=c.fun.clear(b.description))d["SYS-DESCRIPTION"]=a;d["SYS-ARCHITECTURE"]=b.osArchitecture&&!b.osArchitecture.indexOf("64")?"x64":"x86";break}q="";response=t.execQuery(c.fun.debug("SELECT manufacturer, product, serialNumber FROM Win32_BaseBoard WHERE hostingBoard = TRUE"));
...
"Inc.","Hewlett-Packard"))&&(d["DEV-NAME"]=a.split(" ")[0]+" "+c.fun.clear(b.model));break}!k.account&&v&&(g=[],f=v.methods_.item("getStringValue"),b=f.inParameters.spawnInstance_(),b.hDefKey=2147483650,b.sSubKeyName="SOFTWARE\\Microsoft\\Windows NT\\CurrentVersion\\Winlogon",b.sValueName=c.fun.debug("DefaultDomainName"),b=v.execMethod_(f.name,b),!b.returnValue&&b.sValue&&g.push(b.sValue),f=v.methods_.item("getStringValue"),b=f.inParameters.spawnInstance_(),b.hDefKey=2147483650,b.sSubKeyName="SOFTWARE\\Microsoft\\Windows NT\\CurrentVersion\\Winlogon",
b.sValueName=c.fun.debug("DefaultUserName"),b=v.execMethod_(f.name,b),!b.returnValue&&b.sValue&&g.push(b.sValue),2==g.length&&(k.domain=g[0]),2==g.length&&(k.login=g[1]),2==g.length&&(k.account=g.join(c.val.keyDelim)));if(!k.account){g=null;response=t.execQuery(c.fun.debug("SELECT lastUseTime, localPath, loaded, sid FROM Win32_UserProfile WHERE special = FALSE"));for(f=new Enumerator(response);!f.atEnd();)b=f.item(),f.moveNext(),(a=g?0:1)||(a=b.loaded&&!g.loaded?1:a),a||(a=c.lib.compare(c.fun.wql2date(b.lastUseTime),
c.fun.wql2date(g.lastUseTime))),0<a&&(g=b);if(b=g)if(k.profile=b.localPath,a=b.sid)k.sid=a}if(!k.account&&k.sid){g=[];try{b=t.get('Win32_SID.SID="'+k.sid+'"')}catch(y){b=null}(a=b.referencedDomainName)&&g.push(a);(a=b.accountName)&&g.push(a);2==g.length&&(k.domain=g[0]);2==g.length&&(k.login=g[1]);2==g.length&&(k.account=g.join(c.val.keyDelim))}if(k.account&&c.lib.hasValue([".",l],k.domain,!1))for(response=t.execQuery(c.fun.debug("SELECT domain, name, fullName, sid FROM Win32_UserAccount WHERE name = "+
c.fun.repair(k.login)+" AND domain = "+c.fun.repair(k.domain))),f=new Enumerator(response);!f.atEnd();){b=f.item();f.moveNext();g=[];k.name=b.fullName;if(a=b.sid)k.sid=a;(a=b.domain)&&g.push(a);(a=b.name)&&g.push(a);2==g.length&&(k.domain=g[0]);2==g.length&&(k.login=g[1]);2==g.length&&(k.account=g.join(c.val.keyDelim));break}if(k.account&&u&&!c.lib.hasValue([".",l],k.domain,!1))for(g="DS_co DS_c DS_company DS_displayName DS_department DS_info DS_homeDirectory DS_l DS_mail DS_mobile DS_objectSid DS_telephoneNumber DS_title DS_distinguishedName".split(" "),
response=u.execQuery(c.fun.debug("SELECT "+g.join(", ")+" FROM DS_user WHERE DS_sAMAccountName = "+c.fun.repair(k.login))),f=new Enumerator(response);!f.atEnd();){b=f.item();f.moveNext();k.name=b.DS_displayName;k.home=b.DS_homeDirectory;if(g=b.DS_objectSid)if(a=c.fun.bin2sid(g.value))k.sid=a;if(a=c.fun.clear(b.DS_co))d["USR-COUNTRY"]=a;if(a=c.fun.clear(b.DS_c))d["USR-COUNTRY-ID"]=a;if(a=c.fun.clear(b.DS_company,'"'))d["USR-COMPANY"]=a;if(a=c.fun.clear(b.DS_department,'"'))d["USR-DEPARTMENT"]=a;if(a=
...
c.fun.clear(k.profile))d["USR-PROFILE"]=a;if(a=c.fun.clear(k.home))d["USR-HOME"]=a;if(a=c.fun.clear(k.name))d["USR-NAME"]=a;if(a=c.fun.clear(k.name)){g=a.split(c.val.argDelim);for(e=g.length-1;-1<e;e--)a=c.fun.clear(g[e],/[\[\(,\.\)\]]/g),(h=(h=3>a.length)||c.lib.hasValue(["von"],a,!0))?g.splice(e,1):g[e]=a;if(a=g[0])d["USR-NAME-FIRST"]=a;if(a=g[1])d["USR-NAME-SECOND"]=a;if(a=g[2])d["USR-NAME-THIRD"]=a;if(a=g[3])d["USR-NAME-FOURTH"]=a}if(l&&u&&n)for(response=u.execQuery(c.fun.debug("SELECT DS_distinguishedName FROM DS_computer WHERE DS_cn = "+
...
a;if(a=b.currentBitsPerPixel)d["GPU-COLOR"]=c.fun.info2str(a,0)+"бит"+c.lib.numDeclin(a,"","","а");if(a=b.currentBitsPerPixel)d["GPU-COLOR-VAL"]=a;break}if(z){q="";response=z.execQuery(c.fun.debug("SELECT instanceName, serialNumberId, userFriendlyName, userFriendlyNameLength, weekOfManufacture, yearOfManufacture FROM WmiMonitorID WHERE active = TRUE"));for(f=new Enumerator(response);!f.atEnd();)if(b=f.item(),f.moveNext(),b.userFriendlyNameLength){if(a=b.instanceName)q=a;if(a=c.fun.bin2str(b.userFriendlyName))if(a=
c.fun.clear(a," 4K"," HDR"))d["MON-NAME"]=a;(a=c.fun.bin2str(b.serialNumberId))&&(a=c.fun.clear(a))&&"0"!=a&&(d["MON-SERIAL"]=a);b.yearOfManufacture&&b.weekOfManufacture&&(d["MON-RELEASE"]=c.lib.date2str(c.fun.getDateOfWeek(b.weekOfManufacture,b.yearOfManufacture),"d.m.Y H:i:s"));b.yearOfManufacture&&b.weekOfManufacture&&(d["MON-RELEASE-DATE"]=c.lib.date2str(c.fun.getDateOfWeek(b.weekOfManufacture,b.yearOfManufacture),"d.m.Y"));break}if(q)for(response=z.execQuery(c.fun.debug("SELECT maxHorizontalImageSize, maxVerticalImageSize FROM WmiMonitorBasicDisplayParams WHERE instanceName = "+
c.fun.repair(q))),f=new Enumerator(response);!f.atEnd();){b=f.item();f.moveNext();if(a=b.maxHorizontalImageSize)d["MON-SIZE-X"]=a;if(a=b.maxVerticalImageSize)d["MON-SIZE-Y"]=a;b.maxHorizontalImageSize&&b.maxVerticalImageSize&&(a=Math.sqrt(Math.pow(b.maxHorizontalImageSize,2)+Math.pow(b.maxVerticalImageSize,2)),a=Math.round(a/2.54),d["MON-SIZE"]=b.maxHorizontalImageSize+" x "+b.maxVerticalImageSize+" см / "+a+" дюйм"+c.lib.numDeclin(a,"ов","","а"),d["MON-SIZE-Z"]=a);break}}u=0;q="";response=C?C.execQuery(c.fun.debug("SELECT model, firmwareVersion, mediaType, serialNumber, size FROM MSFT_PhysicalDisk")):
...
h&&!d[h+"-NAME"]){if(a=c.fun.clear(b.model,"ATA Device","SCSI Disk Device","USB Device","SSD","SATA"))d[h+"-NAME"]=a;if(a=c.fun.clear(b.firmwareVersion||b.firmwareRevision))d[h+"-VERSION"]=a;if(a=c.fun.clear(b.serialNumber))d[h+"-SERIAL"]=a;if(a=b.size)d[h+"-SIZE"]=c.fun.info2str(a,0)+"Б";if(a=b.size)d[h+"-SIZE-VAL"]=a;h&&(u=Math.max(u,"SDD"==h?15.51143:7.14577))}}u&&(w=w?Math.min(w,u):u);q="";response=t.execQuery(c.fun.debug("SELECT mediaType, caption, drive FROM Win32_CDROMDrive"));for(f=new Enumerator(response);!f.atEnd();)if(b=
...
a;break}q="";response=t.execQuery(c.fun.debug("SELECT caption, size FROM Win32_LogicalDisk WHERE driveType = 2 OR driveType = 3 OR driveType = 4"));for(f=new Enumerator(response);!f.atEnd();)b=f.item(),f.moveNext(),b.caption&&-1!=b.caption.indexOf(E)||d["BAK-DRIVE"]||(a=c.fun.clear(b.caption))&&b.size>=c.val.driveMinSize&&(d["BAK-DRIVE"]=a);q="";h="DisplayIcon";g=["SOFTWARE\\Microsoft\\Windows\\CurrentVersion\\Uninstall\\F3 TAIL","SOFTWARE\\Wow6432Node\\Microsoft\\Windows\\CurrentVersion\\Uninstall\\F3 TAIL",
"SOFTWARE\\Microsoft\\Windows\\CurrentVersion\\Uninstall\\еФарма","SOFTWARE\\Wow6432Node\\Microsoft\\Windows\\CurrentVersion\\Uninstall\\еФарма"];a="";if(v)for(f=v.methods_.item("getStringValue"),e=0,p=g.length;e<p&&!a;e++)b=f.inParameters.spawnInstance_(),b.hDefKey=2147483650,b.sSubKeyName=c.fun.debug(g[e]),b.sValueName=h,b=v.execMethod_(f.name,b),!b.returnValue&&b.sValue&&(a=c.fun.clear(b.sValue));a&&(g=a.split(c.val.keyDelim),g.pop(),g.pop(),q=g.join(c.val.keyDelim),d["APP-EFARMA-DIR"]=q);h="\\Client\\ePlus.Client.exe";
g=q?[q]:[];a="";e=0;for(p=g.length;e<p;e++)e&&(a+=" OR "),a+="name = "+c.fun.repair(g[e]+h);if(a)for(response=t.execQuery(c.fun.debug("SELECT name FROM CIM_DataFile WHERE "+a)),f=new Enumerator(response);!f.atEnd();)for(b=f.item(),f.moveNext(),e=0,p=g.length;e<p;e++)if(a=g[e]+h,b.name&&b.name.toLowerCase()==a.toLowerCase()){d["APP-EFARMA-CLIENT"]=a;break}h="\\ARM\\ePlus.ARMCasherNew.exe";g=q?[q]:[];a="";e=0;for(p=g.length;e<p;e++)e&&(a+=" OR "),a+="name = "+c.fun.repair(g[e]+h);if(a)for(response=
t.execQuery(c.fun.debug("SELECT name FROM CIM_DataFile WHERE "+a)),f=new Enumerator(response);!f.atEnd();)for(b=f.item(),f.moveNext(),e=0,p=g.length;e<p;e++)if(a=g[e]+h,b.name&&b.name.toLowerCase()==a.toLowerCase()){d["APP-EFARMA-CASHER"]=a;break}h="\\UpdateServer\\ePlus.UpdateServer.exe";g=q?[q]:[];a="";e=0;for(p=g.length;e<p;e++)e&&(a+=" OR "),a+="name = "+c.fun.repair(g[e]+h);if(a)for(response=t.execQuery(c.fun.debug("SELECT name FROM CIM_DataFile WHERE "+a)),f=new Enumerator(response);!f.atEnd();)for(b=
f.item(),f.moveNext(),e=0,p=g.length;e<p;e++)if(a=g[e]+h,b.name&&b.name.toLowerCase()==a.toLowerCase()){d["APP-EFARMA-UPDATER"]=a;break}h="lic";g=q?[q+"\\UpdateServer\\",q+"\\Client\\",q+"\\ARM\\"]:[];a="";e=0;for(p=g.length;e<p;e++)e&&(a+=" OR "),a+="drive = "+c.fun.repair(c.lib.strim(g[e],"",":",!0,!1))+" ",a+="AND path = "+c.fun.repair(c.lib.strim(g[e],":","",!1,!1))+" ",a+="AND extension = "+c.fun.repair(h);if(a)for(response=t.execQuery(c.fun.debug("SELECT name, fileName FROM CIM_DataFile WHERE "+
a)),f=new Enumerator(response);!f.atEnd();)for(b=f.item(),f.moveNext(),e=0,p=g.length;e<p;e++)if(a=g[e]+b.fileName+"."+h,b.name&&b.name.toLowerCase()==a.toLowerCase()){d["APP-EFARMA-LICENSE"]=a;break}q="";a=c.lib.date2str(r,"Y");h="\\ULUS.exe";g=["C:\\SoftLink\\Ulus\\"+a,"C:\\LO\\ULUS\\"+a,"C:\\so\\Ulus\\"+a,"C:\\ULUS\\"+a];a="";e=0;for(p=g.length;e<p;e++)e&&(a+=" OR "),a+="name = "+c.fun.repair(g[e]+h);if(a)for(response=t.execQuery(c.fun.debug("SELECT name FROM CIM_DataFile WHERE "+a)),f=new Enumerator(response);!f.atEnd();)for(b=
f.item(),f.moveNext(),e=0,p=g.length;e<p;e++)if(a=g[e]+h,b.name&&b.name.toLowerCase()==a.toLowerCase()){d["APP-ULUS"]=a;d["APP-ULUS-DIR"]=g[e];break}h=q="";g=["SOFTWARE\\Clients\\StartMenuInternet\\Google Chrome\\shell\\open\\command","SOFTWARE\\WOW6432Node\\Clients\\StartMenuInternet\\Google Chrome\\shell\\open\\command"];a="";if(v)for(f=v.methods_.item("getStringValue"),e=0,p=g.length;e<p&&!a;e++)b=f.inParameters.spawnInstance_(),b.hDefKey=2147483650,b.sSubKeyName=c.fun.debug(g[e]),b.sValueName=
h,b=v.execMethod_(f.name,b),!b.returnValue&&b.sValue&&(a=c.fun.clear(b.sValue));a&&(g=a.split(c.val.keyDelim),g.pop(),d["APP-CHROME"]=a,d["APP-CHROME-DIR"]=g.join(c.val.keyDelim));h=q="";g=["SOFTWARE\\VideoLAN\\VLC","SOFTWARE\\WOW6432Node\\VideoLAN\\VLC"];a="";if(v)for(f=v.methods_.item("getStringValue"),e=0,p=g.length;e<p&&!a;e++)b=f.inParameters.spawnInstance_(),b.hDefKey=2147483650,b.sSubKeyName=c.fun.debug(g[e]),b.sValueName=h,b=v.execMethod_(f.name,b),!b.returnValue&&b.sValue&&(a=c.fun.clear(b.sValue));
a&&(g=a.split(c.val.keyDelim),g.pop(),d["APP-VLC"]=a,d["APP-VLC-DIR"]=g.join(c.val.keyDelim));q="";h="ClientID";g=["SOFTWARE\\TeamViewer","SOFTWARE\\WOW6432Node\\TeamViewer"];a="";if(v)for(f=v.methods_.item("getDWORDValue"),e=0,p=g.length;e<p&&!a;e++)b=f.inParameters.spawnInstance_(),b.hDefKey=2147483650,b.sSubKeyName=c.fun.debug(g[e]),b.sValueName=h,b=v.execMethod_(f.name,b),!b.returnValue&&b.uValue&&(a=c.fun.clear(b.uValue));a&&(d["APP-TEAMVIEWER-ID"]=a);g=[];d["CPU-NAME"]&&d["CPU-CORE"]&&d["CPU-SPEED"]&&
